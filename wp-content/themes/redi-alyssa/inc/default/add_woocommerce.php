...
// price vn
function p_wc_price_format_html3(){
 // if ( empty($price) ) return;
...
function p_wc_pd_check_type2( $id ){ 
  ...
  $regular = get_post_meta( $id, 'p_sp1_in_gia', true);
...
function p_wc_tyle( $price, $sale ){
    $tyle = round( ( $price - $sale ) / ( $price /100 ) ) ; 
...
function p_wc_pd_price_sale2( $id ){
  if ( empty($id) ) return;
  ob_get_clean();

  $sale = get_post_meta( $id, 'p_sp1_in_gia', true); 
