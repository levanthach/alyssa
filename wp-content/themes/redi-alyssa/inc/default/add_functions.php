...
Class P_Walker_Nav_Mobile extends Walker_Nav_Menu{
	...
	protected $button = '<i class="fa fa-angle-down child-menu-mobile p-cur"></i>';
...
function p_change_price1( $price = "" ){
   // $price = (int) $price;
    ...
    if (  $price > 1000000000  ){
       ...
       $price_fix = number_format( $price ,0,",","."); 
       // echo "<br/>";

        return trim(substr( $price_fix, 0,  4),'.') . " tỷ";
...
add_action("admin_footer", 'p_get_gia', -1);
...
function p_get_gia(){
	...
	<script  type="text/javascript">
		...
		jQuery(document).ready(function(){
			...
			jQuery('.p_sp1_in_gia2_class input').change(function(){
				
				//alert('sadsad');


				var vl_sale = jQuery(this).val();
				var vl_price = jQuery('.p_sp1_in_gia_class').find('input').val();

				// console.log('vl_sale: ' + vl_sale );

				// console.log('vl_price: ' + vl_price );


				if ( eval(vl_sale) > eval(vl_price)   ) {

					jQuery(this).val( vl_price - 1 );
			...
			jQuery('.p_sp1_in_gia_class input').change(function(){
				
				//alert('sadsad');


				var vl_sale = jQuery('.p_sp1_in_gia2_class').find('input').val(); 
				var vl_price = jQuery(this).val();
				...
				if ( eval(vl_sale) > eval(vl_price)   ) {
					...
					if ( eval(vl_price) == 0  ) {
						jQuery('.p_sp1_in_gia2_class input').val( 0 );

					} else{
						jQuery('.p_sp1_in_gia2_class input').val( vl_price - 1 );
					}
...
function p_myprefix_unregister_tags() {
    unregister_taxonomy_for_object_type('post_tag', 'post');
}
add_action('init', 'p_myprefix_unregister_tags');
